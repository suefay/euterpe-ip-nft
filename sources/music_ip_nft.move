...
/// Euterpe music IP NFT implementation.
module euterpe_ip_nft::music_ip_nft {
    use sui::transfer;
    use sui::event;
    ...
    /// EuterpeMusicIPNFT defines the NFT for the Euterpe music IP.
    struct EuterpeMusicIPNFT has key, store {
        id: UID,
    ...
    public entry fun burn(
        token: EuterpeMusicIPNFT,
        ctx: &mut TxContext
    ) {
        let EuterpeMusicIPNFT { id, url: _ } = token;
